...
sub update_repository {
    ...
    my $artifact = "$artifact_dir/artifact.zip";
...
sub main {
    ...
    # - 4 parse arguments
    my ($repo, $run) = @ARGS;
    die "$program <RUN_URL | OWNER/REPO RUN>" unless defined $repo;

    if ($repo =~ m{^\s*https://.*/([^/]+/[^/]+)/actions/runs/(\d+)(?:/attempts/\d+|)\s*$}) {
        ($repo, $run) = ($1, $2);
    }
    die "$program <RUN_URL | OWNER/REPO RUN>" unless defined $repo && defined $run;
    # - 5 do work
    update_repository($program, $repo, $run);
}
main($0, @ARGV);
